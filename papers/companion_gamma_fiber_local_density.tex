...
\date{2026-05-12 (R753 prime-power lifting addendum, 2026-05-13)}
...
\item Theorem~\ref{thm:BV-Kummer} (R687, the new Gate 0): BV-style cancellation of $\sum_d \lambda_d \Delta_m(d)$ $ongrightarrow$ orbit level of distribution.
\item Pappalardi $\eta = 1/12$ + Heath-Brown identity $K = 4$ + Type I (Lemma~\ref{lem:R686} + Pappalardi) + Type II (Theorem~\ref{thm:BV-Kummer} + FI ASP threshold) $ongrightarrow$ $\sum \Lambda(m 2^k 3^\ell + 1) \to \infty$ $ongrightarrow$ $A(m)$ infinite.
\end{enumerate}
...
%% ===================================================================
\section{Prime-power lifting (R753)}
%% ===================================================================

The body of this note treats squarefree moduli $d=q_1\cdots q_s$.  The general modulus
$d=\prod_i q_i^{e_i}$ requires controlling the vertical tower $q\mid q^2\mid\cdots\mid q^e$
beyond CRT.  We record the exact Hensel-lifting law that makes this tower controllable
by elementary group theory.

Fix rank $r\ge 1$ and units $a_1,\ldots,a_r\in\mathbb Z\setminus\{0\}$ coprime to a fixed odd
prime $q$.  For $e\ge 1$, write
$$
G_{q^e}=(\mathbb Z/q^e\mathbb Z)^*,
\qquad
\Gamma_{q^e}(\mathbf a)=\langle a_1,\ldots,a_r\rangle\subset G_{q^e}.
$$
Reduction $\mathbb Z/q^{e+1}\mathbb Z\twoheadrightarrow\mathbb Z/q^e\mathbb Z$ restricts to a
surjection $\Gamma_{q^{e+1}}(\mathbf a)\twoheadrightarrow\Gamma_{q^e}(\mathbf a)$, since
every generator $a_j\bmod q^{e+1}$ maps to $a_j\bmod q^e$.

\begin{theorem}[Prime-power lifting exact sequence]\label{thm:R753-primepower}
For every $e\ge 1$ there is an exact sequence
$$
1\longrightarrow K_e\longrightarrow
\Gamma_{q^{e+1}}(\mathbf a)\longrightarrow
\Gamma_{q^e}(\mathbf a)\longrightarrow 1,
$$
where
$$
K_e\;=\;\Gamma_{q^{e+1}}(\mathbf a)\;\cap\;
\bigl(1+q^e\mathbb Z/q^{e+1}\mathbb Z\bigr).
$$
The ambient kernel
$1+q^e\mathbb Z/q^{e+1}\mathbb Z\simeq(\mathbb Z/q\mathbb Z,+)$
has prime order $q$.  Consequently
$$
|K_e|\in\{1,\,q\}.
$$
\end{theorem}
...
\begin{proof}
The reduction map $G_{q^{e+1}}\to G_{q^e}$ has kernel
$1+q^e\mathbb Z/q^{e+1}\mathbb Z$, cyclic of order $q$ and canonically isomorphic to the
additive group $\mathbb Z/q\mathbb Z$ via $1+q^e t\mapsto t\bmod q$.  Intersecting this
kernel with $\Gamma_{q^{e+1}}(\mathbf a)$ gives $K_e$.  A subgroup of a group of prime
order has order $1$ or $q$.
\end{proof}

\begin{corollary}[Prime-power subgroup growth]\label{cor:R753-growth}
For every $e\ge 1$,
$$
|\Gamma_{q^{e+1}}(\mathbf a)|=|\Gamma_{q^e}(\mathbf a)|\,|K_e|,
$$
with $|K_e|\in\{1,q\}$.  Iterating from level $1$ to level $e$ gives
$$
|\Gamma_{q^e}(\mathbf a)|=|\Gamma_q(\mathbf a)|\,q^{N_e},
\qquad
0\le N_e\le e-1,
$$
where $N_e$ counts the levels $1\le h<e$ for which $K_h$ is nontrivial.
\end{corollary}

\begin{definition}[Vertical lifting defect]\label{def:vertical-defect}
The vertical lifting defect at level $e$ is
$$
\nu_{q,e}(\mathbf a)=e-1-N_e\;\ge\;0.
$$
It counts the prime-power levels $h<e$ at which $\Gamma_{q^h}(\mathbf a)$ fails to enlarge
by the full Hensel factor $q$ on lifting to $\Gamma_{q^{h+1}}(\mathbf a)$.
\end{definition}

\begin{proposition}[General-modulus defect factorization]\label{prop:R753-general-modulus}
Let $d=\prod_{i=1}^s q_i^{e_i}$ be a general integer with $\gcd(d,a_1\cdots a_r)=1$.  Under
CRT,
$$
|\Gamma_d(\mathbf a)|
\;=\;
\frac{\prod_{i=1}^s|\Gamma_{q_i^{e_i}}(\mathbf a)|}{|\mathcal S_d^{\mathrm{full}}(\mathbf a)|}
\;=\;
\frac{\prod_{i=1}^s|\Gamma_{q_i}(\mathbf a)|\,q_i^{N_{e_i}}}{|\mathcal S_d^{\mathrm{full}}(\mathbf a)|},
$$
where
$$
\mathcal S_d^{\mathrm{full}}(\mathbf a)
\;=\;
\Bigl(\prod_{i=1}^s\Gamma_{q_i^{e_i}}(\mathbf a)\Bigr)\Big/\,\Gamma_d(\mathbf a)
$$
is the synchronization defect quotient at general modulus.
\end{proposition}

\begin{proof}
The first equality is the same Lagrange-quotient argument as in the squarefree case
($|\Gamma_d|=\prod_i|\Gamma_{q_i}|/|\mathcal S_d|$), with $\Gamma_{q_i}$ replaced by
$\Gamma_{q_i^{e_i}}$.  The second equality substitutes Corollary~\ref{cor:R753-growth}
for each prime-power component.
\end{proof}

\begin{remark}[Specialization to $(a,b)=(2,3)$ and the EG203 program]
For the EG203 working case $r=2$, $(a_1,a_2)=(2,3)$, Theorem~\ref{thm:R753-primepower}
gives the explicit Hensel split: at each prime $q\nmid 6$ and each level $e\ge 1$, the
lifting kernel $K_e(2,3)\subset 1+q^e\mathbb Z/q^{e+1}\mathbb Z$ is either trivial or of
order $q$.  Combined with Corollary~\ref{cor:R753-growth} this collapses the prime-power
size question to a single binary at each level.  Coupled with the squarefree formula treated in the body of this note, the general
modulus formula of Proposition~\ref{prop:R753-general-modulus} expresses $|\Gamma_d|$ for
arbitrary $d$ as a product of $|\Gamma_q|$, prime-power Hensel factors $q^{N_e}$, and a
horizontal synchronization quotient $|\mathcal S_d^{\mathrm{full}}|$.  The companion paper
\emph{Rank-$r$ synchronization defects and projective Kummer geometry}
(Wilder, R753 strengthening, 2026-05-13) extends this to bases $\mathbf a=(a_1,\ldots,a_r)$.
\end{remark}
