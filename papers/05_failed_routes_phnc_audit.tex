...
\begin{warning}
This is not a valid plug-in citation.  The BFI theorem concerns primes in arithmetic progressions with well-factorable weights.  At minimum, it is not a plug-in source for this fixed-radical Hecke/Kummer bad-character bound uniformly in \(\ell\); any use would require a precise theorem statement matching this family.
\end{warning}
...
\begin{warning}
This does not by itself imply PHNC uniformly for the growing family
\[
\ell\le(\log Q)^B.
\]
...
\section{Tone and purpose}

This note should be read as an audit ledger, not as a criticism of the cited theorems. The issue in each case is not that the classical theorem is weak or wrong; the issue is that the theorem does not match the exact variable, family, or uniformity requirement of high-seam PHNC without additional work.

\section{What survived the failed routes}

The failure of the PHNC close does not invalidate the following components:

\begin{enumerate}[leftmargin=1.5em]
\item Kummer independence of \(2^r3^s\);
\item nontriviality and exact order \(\ell\) of the Kummer characters;
\item the power-stability lemma;
\item the bad cyclic-line consequence;
\item the projective slope interpretation;
\item the exact identification of the high-\(\ell\) seam.
\end{enumerate}

Thus the audit separates a correct structural component from a missing analytic distribution component.  That separation is the main value of the note.


\section{No-go theorem template}

For every proposed proof route, record:
\begin{enumerate}[leftmargin=1.5em]
\item the claimed input;
\item the exact theorem actually available;
\item the mismatch in variables, range, or family;
\item what the input would imply if true;
\item current status.
\end{enumerate}
This note is an audit ledger, not a claim that no future theorem can close the seam.
